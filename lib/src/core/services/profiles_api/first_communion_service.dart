/*
*   Filename    :   first_communion_service.dart
*   Purpose     :
...
class FirstCommunionService {
  Future<ModuleAndDataActions> getModuleAndDataActions() async {
    List<FirstCommunion> _firstCommunions = [];
    ...
    final urlByIdBase =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.FIRST_COMMUNION_DIR}/?branch_id=$branchId&role_id=$roleId&id=";
      ...
      for (var moduleModel in moduleModels) {
        ...
        await client.get('$urlByIdBase$id').then((respond) {
          if (respond.statusCode == 200) {
            ...
            final firstCommunion = FirstCommunion.fromJson(body);
            _firstCommunions.add(firstCommunion);
          }
        });
      }
    } finally {
    ...
    return ModuleAndDataActions(_firstCommunions, _dataActionModuleModel);
  }
}
