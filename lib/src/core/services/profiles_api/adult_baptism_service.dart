/*
*   Filename    :   adult_baptism_service.dart
*   Purpose     :
...
class AdultBaptismService {
  Future<ModuleAndDataActions> getModuleAndDataActions() async {
    List<AdultBaptism> _adultBaptisms = [];
    ...
    final urlByIdBase =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.ADULT_BAPTISM_DIR}/?branch_id=$branchId&role_id=$roleId&id=";
      ...
      for (var moduleModel in moduleModels) {
        ...
        await client.get('$urlByIdBase$id').then((respond) {
          if (respond.statusCode == 200) {
            ...
            final adultBaptism = AdultBaptism.fromJson(body);
            _adultBaptisms.add(adultBaptism);
          }
        });
      }
    } finally {
    ...
    return ModuleAndDataActions(_adultBaptisms, _dataActionModuleModel);
  }
}
