/*
*   Filename    :   crypt_lobby_service.dart
*   Purpose     :
...
class CryptLobbyService {
  Future<ModuleAndDataActions> getModuleAndDataActions() async {
    ...
    final urlByIdBase =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.CRYPT_LOBBY}/?branch_id=$branchId&role_id=$roleId&id=";
      ...
      for (var moduleModel in moduleModels) {
        ...
        await client.get('$urlByIdBase$id').then((respond) {
          if (respond.statusCode == 200) {
            ...
            final crytpLobby = CryptLobby.fromJson(body);
            _cryptLobbies.add(crytpLobby);
          }
        });
      }
    } finally {
    ...
    return ModuleAndDataActions(_cryptLobbies, _dataActionModuleModel);
  }
}
