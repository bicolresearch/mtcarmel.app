...
class MarriageScreen extends StatefulWidget {
  @override
  _MarriageScreenState createState() => _MarriageScreenState();
}

class _MarriageScreenState extends State<MarriageScreen> {
  List<Marriage> _moduleModels = [];
  ...
  Widget _moduleModelItem(BuildContext context, int index) {
    final Marriage marriage = _moduleAndDataActions.modules[index];
    if (marriage.statusName == "On-going") {
      return Dismissible(
        key: Key(marriage.id),
        child: InkWell(
          child: Container(
...
//          );
  ...
  _updateRequest(Marriage marriage, remarks) async {
    final userId =
...
//    id, status_id, remarks, user_id
    ...
    final url =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.MASS_REQUEST_DIR.split("/")[0]}/update/id/${marriage.id}/role_id/$roleId";
...
//                onChanged: (_){},
  ...
  void _initialize() {
    _moduleAndDataActions =
        BlocProvider.of<MarriageBloc>(context).moduleAndDataActions;
