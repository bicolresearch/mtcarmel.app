/*
*   Filename    :   november_mass_screen.dart
*   Purpose     :
...
import 'package:mt_carmel_app/src/screens/profile_screens/november_mass_screens/november_mass_detail_screen.dart';
import 'package:provider/provider.dart';

enum _SwipedEnum { LeftSwiped, RightSwiped, NotSwiped }

class NovemberMassScreen extends StatefulWidget {
  @override
  _NovemberMassScreenState createState() => _NovemberMassScreenState();
}

class _NovemberMassScreenState extends State<NovemberMassScreen> {
  List<NovemberMass> _moduleModels = [];
  ...
  Widget _moduleModelItem(BuildContext context, int index) {
    final NovemberMass novemberMass = _moduleAndDataActions.modules[index];
    if (novemberMass.statusName == "On-going") {
      return Dismissible(
        key: Key(novemberMass.id),
        child: InkWell(
          child: Container(
            ...
            child: Card(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Text(
                      "Name of dead person: ${_moduleModels[index].name}",
                      style: Theme.of(context)
                          .primaryTextTheme
                          .subtitle
                          .copyWith(fontWeight: FontWeight.bold),
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => MultiProvider(
                  ...
                  child: NovemberMassDetailScreen(),
                ),
              ),
            );
          },
  ...
  _updateRequest(NovemberMass novemberMass, remarks) async {
    final userId =
...
//    id, status_id, remarks, user_id
    ...
    final url =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.MASS_REQUEST_DIR.split("/")[0]}/update/id/${novemberMass.id}/role_id/$roleId";
...
//                onChanged: (_){},
  ...
  void _initialize() {
    _moduleAndDataActions =
        BlocProvider.of<NovemberMassBloc>(context).moduleAndDataActions;
