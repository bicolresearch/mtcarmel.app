/*
*   Filename    :   communion_of_the_sick_screen.dart
*   Purpose     :
...
import 'package:mt_carmel_app/src/screens/profile_screens/communion_of_the_sick_screen/communion_of_the_sick_detail_screen.dart';
import 'package:provider/provider.dart';

enum _SwipedEnum { LeftSwiped, RightSwiped, NotSwiped }

class CommunionOfTheSickScreen extends StatefulWidget {
  @override
  _CommunionOfTheSickScreenState createState() =>
      _CommunionOfTheSickScreenState();
}

class _CommunionOfTheSickScreenState extends State<CommunionOfTheSickScreen> {
  List<CommunionOfTheSick> _moduleModels = [];
  ...
  Widget _moduleModelItem(BuildContext context, int index) {
    final CommunionOfTheSick communionOfTheSick =
        _moduleAndDataActions.modules[index];
    if (communionOfTheSick.statusName == "On-going") {
      return Dismissible(
        key: Key(communionOfTheSick.id),
        child: InkWell(
          child: Container(
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => MultiProvider(
                  providers: [
                    Provider.value(
                      value: communionOfTheSick,
                    ),
                    Provider<String>.value(value: _serviceName),
                  ],
                  child: CommunionOfTheSickDetailScreen(),
                ),
              ),
            );
          },
  ...
  _updateRequest(CommunionOfTheSick communionOfTheSick, remarks) async {
    final userId =
...
//    id, status_id, remarks, user_id
    ...
    final url =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.MASS_REQUEST_DIR.split("/")[0]}/update/id/${communionOfTheSick.id}/role_id/$roleId";
...
//                onChanged: (_){},
  ...
  void _initialize() {
    _moduleAndDataActions =
        BlocProvider.of<CommunionOfTheSickBloc>(context).moduleAndDataActions;
