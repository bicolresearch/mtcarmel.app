...
class FuneralServiceScreen extends StatefulWidget {
  @override
  _FuneralServiceScreenState createState() => _FuneralServiceScreenState();
}

class _FuneralServiceScreenState extends State<FuneralServiceScreen> {
  List<FuneralService> _moduleModels = [];
  ...
  Widget _moduleModelItem(BuildContext context, int index) {
    final FuneralService funeralService = _moduleAndDataActions.modules[index];
    if (funeralService.statusName == "On-going") {
      return Dismissible(
        key: Key(funeralService.id),
        child: InkWell(
          child: Container(
...
//          );
  ...
  _updateRequest(FuneralService funeralService, remarks) async {
    final userId =
...
//    id, status_id, remarks, user_id
    ...
    final url =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.MASS_REQUEST_DIR.split("/")[0]}/update/id/${funeralService.id}/role_id/$roleId";
...
//                onChanged: (_){},
  ...
  void _initialize() {
    _moduleAndDataActions =
        BlocProvider.of<FuneralServiceBloc>(context).moduleAndDataActions;
