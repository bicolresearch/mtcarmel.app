...
import 'package:mt_carmel_app/src/screens/profile_screens/funeral_chapel_screens/funeral_chapel_detail_screen.dart';
import 'package:provider/provider.dart';

enum _SwipedEnum { LeftSwiped, RightSwiped, NotSwiped }

class FuneralChapelScreen extends StatefulWidget {
  @override
  _FuneralChapelScreenState createState() => _FuneralChapelScreenState();
}

class _FuneralChapelScreenState extends State<FuneralChapelScreen> {
  List<FuneralChapel> _moduleModels = [];
  ...
  Widget _moduleModelItem(BuildContext context, int index) {
    final FuneralChapel funeralChapel = _moduleAndDataActions.modules[index];
    if (funeralChapel.statusName == "On-going") {
      return Dismissible(
        key: Key(funeralChapel.id),
        child: InkWell(
          child: Container(
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => MultiProvider(
                  ...
                  child: FuneralChapelDetailScreen(),
                ),
              ),
            );
          },
...
//          );
  ...
  _updateRequest(FuneralChapel funeralChapel, remarks) async {
    final userId =
...
//    id, status_id, remarks, user_id
    ...
    final url =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.MASS_REQUEST_DIR.split("/")[0]}/update/id/${funeralChapel.id}/role_id/$roleId";
...
//                onChanged: (_){},
  ...
  void _initialize() {
    _moduleAndDataActions =
        BlocProvider.of<FuneralChapelBloc>(context).moduleAndDataActions;
