...
import 'package:mt_carmel_app/src/models/crypt_lobby.dart';
import 'package:mt_carmel_app/src/models/funeral_service.dart';
import 'package:mt_carmel_app/src/presentations/mount_carmel_icons.dart';
import 'package:mt_carmel_app/src/screens/profile_screens/crypt_lobby_srceens/crypt_lobby_detail_screen.dart';
...
class CryptLobbyScreen extends StatefulWidget {
  @override
  _CryptLobbyScreenState createState() => _CryptLobbyScreenState();
}

class _CryptLobbyScreenState extends State<CryptLobbyScreen> {
  List<CryptLobby> _moduleModels = [];
  ...
  Widget _moduleModelItem(BuildContext context, int index) {
    final CryptLobby cryptLobby = _moduleAndDataActions.modules[index];
    if (cryptLobby.statusName == "On-going") {
      return Dismissible(
        key: Key(cryptLobby.id),
        child: InkWell(
          child: Container(
...
//          );
  ...
  _updateRequest(CryptLobby cryptLobby, remarks) async {
    final userId =
...
//    id, status_id, remarks, user_id
    ...
    final url =
        "${AppConstants.API_BASE_URL}${ModuleDirectories.MASS_REQUEST_DIR.split("/")[0]}/update/id/${cryptLobby.id}/role_id/$roleId";
...
//                onChanged: (_){},
  ...
  void _initialize() {
    _moduleAndDataActions =
        BlocProvider.of<CryptLobbyBloc>(context).moduleAndDataActions;
